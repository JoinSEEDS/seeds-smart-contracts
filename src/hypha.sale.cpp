...
asset sale::seeds_for_usd(asset usd_quantity) {
  update_price();
...
// void sale::updateusd(asset seeds_per_usd) {
//   require_auth(get_self());
...
ACTION sale::addround(uint64_t volume, asset seeds_per_usd) {
  require_auth(get_self());
...
ACTION sale::initrounds(uint64_t volume_per_round, asset initial_seeds_per_usd) {
  require_auth(get_self());
