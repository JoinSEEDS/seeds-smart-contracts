...
describe('Test migration', async assert => {
  ...
  const day = parseInt(Date.now() / 1000)
  ...
  const getNewDailyTransactions = async (day) => {
    ...
    console.log(trxs.rows)
    return trxs.rows
  }

  const trxFirstuser = await getTransactions(firstuser)
  const trxSeconduser = await getTransactions(seconduser)
  const trxFirstorg = await getTransactions(firstorg, 'org')
  const trxSecondorg = await getTransactions(secondorg, 'org')

  await contracts.history.migrateusers({ authorization: `${history}@active` })
  await sleep(2000)

  const todayTransactions = await getNewDailyTransactions(day)
