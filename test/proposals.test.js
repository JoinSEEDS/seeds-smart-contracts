...
describe('Test props migration', async assert => {
  ...
  for (let i = 0; i < users.length; i++) {
    await contracts.accounts.adduser(users[i], `user ${i}`, 'individual', { authorization: `${accounts}@active` })
    await contracts.accounts.testcitizen(users[i], { authorization: `${accounts}@active` })
    await contracts.proposals.addvoice(users[i], 99, { authorization: `${proposals}@active` })
  }

  const createProposal = async (number, bank) => {
    await contracts.proposals.create(firstuser, firstuser, '12.0000 SEEDS', 'campaign', `test campaign ${number}`, 'description', 'image', 'url', bank, { authorization: `${firstuser}@active` })
    await contracts.token.transfer(firstuser, proposals, '555.0000 SEEDS', `${number}`, { authorization: `${firstuser}@active` })
  }

  const voteProposal = async (propId, approve=true) => {
    if (approve) {
      await contracts.proposals.favour(seconduser, propId, 1, { authorization: `${seconduser}@active` })
      await contracts.proposals.favour(thirduser, propId, 1, { authorization: `${thirduser}@active` })
      await contracts.proposals.favour(fourthuser, propId, 1, { authorization: `${fourthuser}@active` })
    } else {
      await contracts.proposals.against(seconduser, propId, 1, { authorization: `${seconduser}@active` })
      await contracts.proposals.against(thirduser, propId, 1, { authorization: `${thirduser}@active` })
      await contracts.proposals.against(fourthuser, propId, 1, { authorization: `${fourthuser}@active` })
    }
  }

  console.log('create campaign proposal')
  await createProposal(1, campaignbank)
  await createProposal(2, campaignbank)
  await createProposal(3, alliancesbank)
  await createProposal(4, alliancesbank)
  await createProposal(5, alliancesbank)
 
  console.log('active proposals')
  await contracts.proposals.onperiod({ authorization: `${proposals}@active` })
  await sleep(3000)

  const voicesP = users.map(user => { return contracts.proposals.addvoice(user, 40, { authorization: `${proposals}@active` }) })
  await Promise.all(voicesP)

  const voices = await eos.getTableRows({
    code: proposals,
    scope: proposals,
    table: 'voice',
    json: true,
  ...
  await voteProposal(2, false)
  await voteProposal(3)
  await voteProposal(4, false)
  await voteProposal(5)

  console.log('active proposals')
  await contracts.proposals.onperiod({ authorization: `${proposals}@active` })
  await sleep(3000)

  await createProposal(6, campaignbank)
  await createProposal(7, alliancesbank)

  console.log('migrating prop table')
  await contracts.proposals.migprops(0, { authorization: `${proposals}@active` })

  await sleep(6000)

  const migrateProps = await eos.getTableRows({
    code: proposals,
    scope: proposals,
    table: 'migrateprops',
    json: true,
  })
  console.log(migrateProps)

  console.log('deleting old proposals table')
  await contracts.proposals.resetprops({ authorization: `${proposals}@active` })
  await sleep(4000)
